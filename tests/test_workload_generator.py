...
def test_generate_new_workload(
    ...
    generator = WorkloadGenerator(config_path=workload_config_path, workload=workload)
    generator.execute()

    assert mock_mkdir.called is True

    assert mock_file.call_args_list == [
        call(f"{workload_config_path}/workload-1/info.yaml", "w"),
        call(f"{workload_config_path}/workload-1/development.yaml", "w"),
        call(f"{workload_config_path}/workload-1/testing.yaml", "w"),
        call(f"{workload_config_path}/workload-1/acceptance.yaml", "w"),
        call(f"{workload_config_path}/workload-1/production.yaml", "w"),
    ]
...
def test_generate_existing_workload(
    ...
    assert mock_file.call_args_list == [
        call(f"{workload_config_path}/workload-1/info.yaml", "r"),
        call(f"{workload_config_path}/workload-1/info.yaml", "w"),
        call(),
    ]
